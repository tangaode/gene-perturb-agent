...
def main():
    ...
    markers = compute_top_markers(Xn, genes, labels, top_n=args.top_markers)
    um = run_umap(pcs)
    ...
    save_outputs(args.out_dir, barcodes, labels, um, markers, cell_map)

    # print compact summary for launcher parsing
    uniq = sorted(list(set(labels.tolist())))
    summary = {
        "out_dir": str(Path(args.out_dir).resolve()),
        "clusters": [{"cluster": int(c), "cell_type": cell_map.get(int(c), f"cluster_{c}"), "n_cells": int((labels == c).sum())} for c in uniq],
